...
def calculate_amplitude_of_IPSP(spw_ipsps_trace, data_trace, fs):
    """ calculate the maximum amplitude of each IPSP, fr last IPSP return 0"""
    ...
    ipsps_starts = time_pts[:-1].copy()
    ipsps_ends = time_pts[1:].copy()
    
    last_ipsp = np.append(np.diff(spw_ipsps_sorted['electrode'])!=0, [True])
    
    ipsps_ends[last_ipsp[:-1]] = (ipsps_starts[last_ipsp[:-1]] + 2).copy()
    
    ipsp_ampl = [np.max(data_trace[spw_ipsps_sorted['electrode'][i], ipsps_starts[i]:ipsps_ends[i]])
                 for i in range(len(spw_ipsps_sorted) - 1)]
    
    ipsps_ampl = np.concatenate([ipsp_ampl, [0]])
    ipsps_ampl_less_begining = ipsps_ampl - data_trace[spw_ipsps_sorted['electrode'],time_pts]
    ipsps_ampl_less_begining[last_ipsp] = 0
...
def update_spws_beg(load_datafile, load_spwsipsp, load_spwsspike, save_folder, save_fig, save_file,ext):
    ...
    for trace in all_traces:
        ...
        # if there is spike, use the spike as the beginning of the IPSP
