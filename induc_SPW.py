...
#    del data_filt, starts
    ...
    spw_ipsps = spw_ipsps[(spw_ipsps['trace'] == use_trace) & (spw_ipsps['ipsp_start'] >= spw_zoom[0]) & (spw_ipsps['ipsp_start'] <= spw_zoom[1])]
    ...
    for idx, electr in enumerate(use_electrodes):
        ...
        ipsp_raw_electr = spw_ipsps[spw_ipsps['electrode'] == electr-1]
        #import pdb; pdb.set_trace()
        ...
        for lines in range(len(ipsp_raw_pts)):
            #
            plt.vlines(t_zoom[ipsp_raw_pts[lines]], ipsp_line_place, data_electr[idx, ipsp_raw_pts[lines]], linestyle=':')
        ...
        spikes_largest_electr = spikes_largest[spikes_largest['electrode'] == electr-1]
        spike_largestpts = ms2pts(spikes_largest_electr['time'] - spw_zoom[0], fs).astype('i4')
        plt.plot(t_zoom[spike_largestpts], np.ones(len(spike_largestpts))*-250, 'r|', lw = 5, ms = 8)
        plt.ylim([-300,300])
        #plt.plot(t_zoom[spike_largestpts], data_electr[idx, spike_largestpts]  - 20 + add_it * idx, 'r^', lw = line_width, ms = 5)
    #import pdb; pdb.set_trace()
    plt.xlim([t_zoom[0], t_zoom[0] + 70]) #t_zoom[-1]])     
    ...
    change_size = move_start_pts - move_end_pts
    data_electr = np.zeros([len(data_trace), len(t_zoom) - change_size])
    spw_zoom = [spw_zoom[0]+move_start,spw_zoom[1]+move_end]
    spw_zoom_pts = [spw_zoom_pts[0]+move_start_pts,spw_zoom_pts[1]+move_end_pts]
    t_zoom = dat.get_timeline(data_trace[0, spw_zoom_pts[0]: spw_zoom_pts[1]], fs, 'ms') + spw_zoom[0]
    for electr in range(1, len(data_trace) + 1):
        idx = electr - 1
        #import pdb; pdb.set_trace()
        data_electr[idx, :] = data_trace[electr-1, spw_zoom_pts[0]: spw_zoom_pts[1]]
        plt.plot(t_zoom, data_electr[idx, :] + add_it * idx, lw = 1, c = 'k')
        plt.text(t_zoom[0]-2, add_it * idx, str(electr))
    # plot start of SPW
    #import pdb; pdb.set_trace()

    
    # plot IPSPs
    number_of_ipsps = len(np.unique(spw_ipsps['group']))
    all_ipsps = np.unique(spw_ipsps['group'])
    ipsp_times = []
    for ipsp_no in all_ipsps:
        ipsp_time = min(spw_ipsps[spw_ipsps['group'] == ipsp_no]['ipsp_start'])
        ipsp_times.append(ipsp_time)
    sorted = np.argsort(ipsp_times)
    #import pdb; pdb.set_trace() 
    all_ipsps = all_ipsps[sorted]
    calc_text = ""
    for idx,ipsp_no in enumerate(all_ipsps):
        #import pdb; pdb.set_trace()    
        ipsp_run = spw_ipsps[spw_ipsps['group'] == ipsp_no]
        min_electr = min(ipsp_run['electrode'])
        max_electr = max(ipsp_run['electrode'])
        
        numb_it = max_electr - min_electr + 1
        ipsp_time = ipsp_run['ipsp_start'][0]
        ipsp_start = ms2pts(ipsp_time, fs).astype('i4')
        calc_text = calc_text + str(numb_it)
        if idx < number_of_ipsps-1:
            next_ipsp_no = all_ipsps[idx+1]

            ipsp_run = spw_ipsps[spw_ipsps['group'] == next_ipsp_no]['ipsp_start'][0]
            ipsp_end = ms2pts(ipsp_run, fs).astype('i4')
            calc_text = calc_text + " + "
        else:
            ipsp_end = ipsp_start + ms2pts(5,fs).astype('i4')

        electr_max = np.argmax(np.max(data_trace[:,ipsp_start:ipsp_end],1))
        move_pts = np.argmax(data_trace[electr_max,ipsp_start:ipsp_end])

        move_ms = pts2ms(move_pts, fs)
        
        #if idx ==6:
        #    import pdb; pdb.set_trace()
        min_pt = add_it * min_electr + data_trace[min_electr,ipsp_start + move_pts] - 20
    ...
    # plot start of SPW
    min_plot = 0 - add_it * 1
    max_plot = len(data_trace) * add_it + add_it* 3
...
def update_add_missing_electrodes_SPW(save_folder, save_file, spw_file, data_file):
    ...
    idx_spw = 0
    win_base = [-3, -1]
    win_base_pts = [ms2pts(win_base[0], fs),ms2pts(win_base[1], fs)]
        ...
        if sum(np.diff(group_times) > ipsp_min_len) > 1:
            ...
            for idx_far in idx_used_far:
                ...
                if np.diff(group_times)[idx_far] > separate_if_ipsps_further:
                    ...
                    #import pdb; pdb.set_trace()
                    data_spw = data_trace[:, ms2pts(spw_start, fs):end_til_now]
                    if remove_baseline:
                        # remove baseline
                        base_start = spw_start_pts + win_base_pts[0]
                        base_end = spw_start_pts + win_base_pts[1]
                        data_spw = remove_baseline_spw(data_trace, data_spw, base_start, base_end)
                    
                    max_this_trace = np.max(data_spw)
...
def update_fill_gap_between_ipsp_groups(save_folder, save_file, spw_file, data_file):
    ...
    for trace in range(np.size(data,1)):
        ...
        for spw_no in np.unique(all_ipsps['spw_no'] [all_ipsps['trace'] == trace]):
            new_start = min(all_ipsps['ipsp_start'] [all_ipsps['spw_no'] == spw_no])
            #if new_start != all_ipsps['spw_start'][all_ipsps['spw_no'] == spw_no][0]:
                #import pdb; pdb.set_trace()
            all_ipsps['spw_start'][all_ipsps['spw_no'] == spw_no] = np.ones(len(all_ipsps['spw_start'][all_ipsps['spw_no'] == spw_no])) * new_start
            #import pdb; pdb.set_trace()
...
def calculate_amplitude_of_IPSP(spw_ipsps_trace, data_trace, fs, add_to_start = 0.0):
    ...
    time_pts = ms2pts(spw_ipsps_sorted['ipsp_start'] + add_to_start, fs).astype(np.uint32)
...
def calculate_max_in_given_patch(data, points, distanse_from_point, fs, add_to_start = 0.0):
    """ calculates the amplitude of the IPSPs given"""
    index = np.argsort(points, order=['electrode', 'ipsp_start'])
    #index = np.argsort(points, order=['ipsp_start'])
    points_ordered = points[index]
    electr = points_ordered['electrode'][0]
    points_pts = ms2pts(points_ordered['ipsp_start'] + add_to_start, fs).astype('i4')
...
def remove_baseline_spw(data_used, data_spw, base_start, base_end):
    ...
    base = data_used[:, base_start: base_end]
...
def update_remove_too_small_spws(load_datafile, load_spwsipsp, min_ampl, save_folder, save_file, ext):
    ...
    for idx, spw_no in enumerate(spw_no_all):
        ...
        accept = 5
        spw_used = spws[spws['spw_no'] == spw_no]
        spw_used['spw_start'] = min(spw_used['ipsp_start'])
        ipsp_no = len(np.unique(spw_used['group']))
        while accept > 0 and ipsp_no > 0:
            ...
            spw_start_pts = ms2pts(spw_start + add_to_start, fs).astype('i4')
            ...
            if spw_start_pts < spw_end_pts + 2:
                ...
                if max_spw >= min_ampl:
                    ...
                    min_ipsp_group = spw_used[spw_used['ipsp_start'] == min(spw_used['ipsp_start'])]['group'][0] 
                    #first_ipsp = spw_used[spw_used['group']==first_ipsp]
                    spw_used['spw_start'] = np.max(spw_used[spw_used['group']==min_ipsp_group]['ipsp_start'])
                    new_spws.append(spw_used)
                    new_spw_no = new_spw_no + 1
                    accept = 0
                elif ipsp_no > 1:
                    # remove the first IPSP and try with the next one
                    accept = accept - 1
                    #import pdb; pdb.set_trace() 
                    min_ipsp_group = spw_used[spw_used['ipsp_start'] == spw_start]['group'][0]
                    spw_used = spw_used[spw_used['group'] != min_ipsp_group]
                    #import pdb; pdb.set_trace() 
                    spw_used['spw_start'] = min(spw_used['ipsp_start'])
                else:
                    accept = accept - 1
...
def corect_ipsps(load_datafile, load_spwsipsp, load_spwsspike, save_folder, save_fig, save_file,ext):
    ...
    min_length_ipsp = 3 # ipsp cannot be shorter than this
    min_ampl_ipsp = 10
    ...
    for trace in all_traces:
        ...
        #import pdb; pdb.set_trace()
        # remove IPSPs which are of too low amplitude
        ipsps_ampls = calculate_amplitude_of_IPSP(spw_ipsps_trace, data_trace, fs, add_to_start = 0.25)
        spw_ipsps_trace = spw_ipsps_trace[[ipsps_ampls > min_ampl_ipsp] or [ipsps_ampls == 0]]
...
def update_spws_beg(load_datafile, load_spwsipsp, load_spwsspike, save_folder, save_fig, save_file,ext, win = [-20, 80], expected_min_ipsp_ampl = 30):
    ...
    # define parameters
    distanse_from_point = 3 # ms - length of IPSP
    shift_ipsp = 2.5 # ms
    ...
    for trace in all_traces:
        ...
        for spw_no in np.unique(spw_ipsps_trace['spw_no']):
            ...
            #import pdb; pdb.set_trace()
            max_ampls = calculate_max_in_given_patch(data_trace, spw_ipsps_spw_no[['electrode','ipsp_start']], distanse_from_point, fs, add_to_start = 0.25)
            #max_ampls = calculate_amplitude_of_IPSP(spw_ipsps_spw_no, data_trace, fs, add_to_start = 0.25)
            # check which IPSPs are of too low amplitude
            spw_ipsps_first = spw_ipsps_spw_no[max_ampls >= expected_min_ipsp_ampl]
            spw_ipsps_spw_no = spw_ipsps_spw_no[max_ampls >= expected_min_ipsp_ampl]
            ...
            if len(spw_ipsps_first) > 0: #& min_ipsp_groups:
                ...
                #ipsp_amplitudes = calculate_amplitude_of_IPSP(spw_ipsps_spw_no, data_trace, fs, add_to_start = 0.25)
...
def update_spws_ipsp_beg(load_datafile, filter_folder, load_spwsipsp, load_spwsspike, save_folder, save_fig, save_file,ext, win = [-20, 80], save_filter = 'ipsp_filt_', expected_min_ipsp_ampl = 30):
    """ checks all the ipsps and corrects them for each spw"""
    ...
    # microV, min allowed IPSP amplitude
        ...
        for electr in range(np.size(data,0)):
            ...
            for idx, m in enumerate(meanEach):
                if m * switch > (mean_temp + err_allowed)*switch:
                    ...
                    detected_max = np.max(data_used[(idx-1)*window : (idx+1)*window] * switch)
                    if switch >0 and detected_max >= min_ipsp_ampl:
            ...
            #maxs = np.unique(maxs).astype('i4') #use only Richard's idea
